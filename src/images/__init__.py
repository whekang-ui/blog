"""이미지 준비: AI 비임상 이미지 + 인포그래픽 자동생성 + 사용자 사진 + alt 텍스트.

본문의 [[IMAGE:설명]] 토큰 각각을 실제 이미지 파일에 매핑한다. 매핑 우선순위(기본):
1) 사용자가 user_photos/ 에 넣고 지정한 사진
2) 인포그래픽 자동생성(설명이 '정보/과정/주의사항/비교' 등 정보성일 때)
3) AI 비임상 이미지 생성(개념/일러스트/스킨케어 모델 느낌) — ai_images_enabled + 키 있을 때
4) 위가 모두 불가하면 인포그래픽으로 폴백

각 슬롯이 왜 그 종류가 됐는지(사유)를 log 콜백으로 알려줘, "왜 인포만 나오나"를 즉시 확인 가능.
"""

from __future__ import annotations

from dataclasses import dataclass
from typing import Callable

from ..config import Config
from .ai_image import generate_ai_image_verbose
from .alt_text import make_alt_text
from .infographic import render_infographic

_INFOGRAPHIC_HINTS = ("정보", "과정", "단계", "주의", "비교", "표", "체크", "요약", "통계")


@dataclass
class PreparedImage:
    description: str
    path: str
    kind: str  # 'user' | 'infographic' | 'ai'
    alt_text: str
    position: int


def prepare_media(
    config: Config,
    descriptions: list[str],
    *,
    user_photo_map: dict[int, str] | None = None,
    log: Callable[[str], None] | None = None,
) -> list[PreparedImage]:
    """본문 이미지 설명 목록을 실제 이미지로 변환한다.

    user_photo_map: {본문 이미지 인덱스(0부터): 사용자 사진 파일경로}
    log: 슬롯별 처리 사유를 출력할 콜백(선택).
    """
    user_photo_map = user_photo_map or {}
    out_dir = config.media_output_dir()
    ai_enabled = bool(config.get("images.ai_images_enabled", False)) and config.secrets.has_image_api
    info_enabled = bool(config.get("images.infographics_enabled", True))

    def _note(msg: str) -> None:
        if log:
            log(msg)

    prepared: list[PreparedImage] = []
    for idx, desc in enumerate(descriptions):
        # 1) 사용자가 직접 지정한 사진
        if idx in user_photo_map:
            path = user_photo_map[idx]
            prepared.append(PreparedImage(desc, path, "user", make_alt_text(config, desc), idx))
            _note(f"[{idx}] 내 사진 사용: {path}")
            continue

        # 2) 정보성 설명이면 인포그래픽
        if info_enabled and _looks_informational(desc):
            path = render_infographic(config, desc, out_dir / f"info_{idx}.png")
            prepared.append(
                PreparedImage(desc, str(path), "infographic", make_alt_text(config, desc), idx)
            )
            _note(f"[{idx}] 인포그래픽 (설명이 정보성: '{desc[:20]}…')")
            continue

        # 3) AI 비임상 이미지
        if ai_enabled:
            path, reason = generate_ai_image_verbose(config, desc, out_dir / f"ai_{idx}.png")
            if path:
                prepared.append(
                    PreparedImage(desc, str(path), "ai", make_alt_text(config, desc), idx)
                )
                _note(f"[{idx}] AI 이미지 생성 성공")
                continue
            _note(f"[{idx}] AI 실패→인포그래픽 폴백: {reason}")
        elif not ai_enabled:
            why = "ai_images_enabled=false" if not config.get("images.ai_images_enabled", False) \
                else "이미지 API 키 미설정"
            _note(f"[{idx}] AI 미사용({why}) → 인포그래픽")

        # 4) 폴백: 인포그래픽
        if info_enabled:
            path = render_infographic(config, desc, out_dir / f"info_{idx}.png")
            prepared.append(
                PreparedImage(desc, str(path), "infographic", make_alt_text(config, desc), idx)
            )

    return prepared


def _looks_informational(description: str) -> bool:
    return any(h in description for h in _INFOGRAPHIC_HINTS)
