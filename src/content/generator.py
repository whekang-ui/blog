...
def generate_post(
    ...
    *,
    ...
    body_prompt = (
        ...
        "본문 흐름에 맞게 3개 이상 넣되, 다음 비율로 구성하라:\n"
        "  - 1~2개: **비임상 콘셉트/모델 이미지**(AI 생성용). 예: '맑고 윤기나는 피부의 한국 여성 "
        "모델', '깨끗한 욕실의 스킨케어 제품 컷', '세안 후 얼굴에 크림을 바르는 손', "
        "'편안하게 미소 짓는 모델의 건강한 피부'. 이런 설명에는 '인포그래픽/비교/단계' 같은 단어를 "
        "쓰지 말 것(그러면 표로 처리됨).\n"
        "  - 1개: **정보성 인포그래픽**. 예: '핵심 관리 단계 요약 인포그래픽'.\n"
        "  - 금지: 환자/시술 전후/병변/수술 장면 등 임상 사진 설명은 절대 만들지 말 것"
        "(의료법 위반이며 AI 생성도 차단됨).\n"
