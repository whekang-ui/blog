...
- `HF 주소를 찾지 못함(DNS)` 가 뜨면 huggingface.co 접속이 막힌 것입니다(병원/회사 방화벽·DNS).
  브라우저로 huggingface.co 가 열리는지 확인하거나 다른 네트워크에서 시도하세요. 엔드포인트는
  `images.hf_endpoint`(기본 `router.huggingface.co`)로 바꿀 수 있습니다.
